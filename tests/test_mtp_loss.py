...
def test_multi_token_prediction_head_returns_stacked_future_logits() -> None:
    head = MultiTokenPredictionHead(d_model=8, vocab_size=13, horizons=(2, 3, 4))
...
def test_multi_token_prediction_head_backpropagates_to_hidden_states() -> None:
    head = MultiTokenPredictionHead(d_model=8, vocab_size=13, horizons=(2, 3))
...
def test_multi_token_cross_entropy_matches_manual_shifted_targets() -> None:
    ...
    logits = torch.randn(2, 1, 6, 7)

    loss, per_horizon = multi_token_cross_entropy(
        logits,
        token_ids,
        horizons=(2, 3),
    )

    horizon_2 = F.cross_entropy(logits[0, :, :-2, :].reshape(-1, 7), token_ids[:, 2:].reshape(-1))
    horizon_3 = F.cross_entropy(logits[1, :, :-3, :].reshape(-1, 7), token_ids[:, 3:].reshape(-1))
    expected = torch.stack([horizon_2, horizon_3]).mean()

    assert torch.allclose(loss, expected)
    assert per_horizon == pytest.approx((float(horizon_2.item()), float(horizon_3.item())))
...
def test_multi_token_cross_entropy_backpropagates_to_future_logits() -> None:
    ...
    loss, _ = multi_token_cross_entropy(logits, token_ids, horizons=(2, 3))
    loss.backward()
...
def test_multi_token_cross_entropy_rejects_invalid_token_rank() -> None:
    with pytest.raises(ValueError, match="token_ids must have shape"):
        multi_token_cross_entropy(
            torch.randn(2, 2, 5, 7),
            torch.ones(2, 5, 1, dtype=torch.long),
            horizons=(2, 3),
        )
...
def test_multi_token_cross_entropy_rejects_horizon_at_or_above_sequence_length() -> None:
    with pytest.raises(ValueError, match="horizon must be smaller than sequence length"):
        multi_token_cross_entropy(
            torch.randn(2, 2, 5, 7),
            torch.ones(2, 5, dtype=torch.long),
            horizons=(2, 5),
...
def test_multi_token_cross_entropy_rejects_duplicate_next_token_horizon() -> None:
...
def test_baseline_gpt_forward_mtp_returns_main_and_future_logits() -> None:
    ...
    assert output.future_token_logits.shape == (
        len(config.mtp_horizons),
        2,
        5,
        config.vocab_size,
    )
    assert output.horizons == (2, 3)
