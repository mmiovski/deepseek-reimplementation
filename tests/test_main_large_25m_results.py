...
def test_export_main_large_25m_summary_cli() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/analysis/export_main_large_25m_summary.py"],
        check=True,
        capture_output=True,
        text=True,
    )
    assert "main_large_25m_fixed_budget_results" in result.stdout


def test_main_large_25m_summary_has_all_six_main_runs() -> None:
    summary_path = Path("results/analysis/main_large_25m_summary.json")
    csv_path = Path("results/analysis/main_large_25m_summary.csv")

    assert summary_path.exists()
    assert csv_path.exists()

    payload = json.loads(summary_path.read_text(encoding="utf-8"))
    assert payload["scope"] == "main_large_25m_fixed_budget_results"
    assert payload["requested_train_tokens"] == 25_000_000
    ...
    for row in payload["runs"]:
        assert row["requested_train_tokens"] == 25_000_000
        assert row["train_tokens"] >= 25_000_000
...
def test_main_large_25m_summary_uses_exact_model_specific_parameters() -> None:
    payload = json.loads(
        Path("results/analysis/main_large_25m_summary.json").read_text(encoding="utf-8")
    )
