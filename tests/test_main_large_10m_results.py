...
def test_export_main_large_10m_summary_cli() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/analysis/export_main_large_10m_summary.py"],
        check=True,
        capture_output=True,
        text=True,
    )
    assert "main_large_10m_fixed_budget_results" in result.stdout


def test_main_large_10m_summary_has_all_six_main_runs() -> None:
    summary_path = Path("results/analysis/main_large_10m_summary.json")
    csv_path = Path("results/analysis/main_large_10m_summary.csv")

    assert summary_path.exists()
    assert csv_path.exists()

    payload = json.loads(summary_path.read_text(encoding="utf-8"))
    assert payload["scope"] == "main_large_10m_fixed_budget_results"
    assert payload["requested_train_tokens"] == 10_000_000
    ...
    for row in payload["runs"]:
        assert row["requested_train_tokens"] == 10_000_000
        assert row["train_tokens"] >= 10_000_000
...
def test_main_large_10m_summary_uses_exact_model_specific_parameters() -> None:
    payload = json.loads(
        Path("results/analysis/main_large_10m_summary.json").read_text(encoding="utf-8")
    )
    ...
    assert params_by_model["baseline_gpt"] == (
        120_945_408,
        120_945_408,
        120_945_408,
    )
    assert params_by_model["mla_gpt"] == (
        137_460_480,
        137_460_480,
        137_460_480,
    )
    assert params_by_model["mtp_gpt"] == (
        136_305_408,
        136_305_408,
        136_305_408,
    )
    assert params_by_model["moe_gpt"] == (
        220_146_432,
        220_146_432,
        78_588_672,
    )
    assert params_by_model["mla_moe_gpt"] == (
        236_661_504,
        236_661_504,
        95_103_744,
    )
    assert params_by_model["v3_routing_gpt"] == (
        220_146_432,
        220_146_432,
        78_588_672,
    )
