"""Export curated main-large 25M fixed-budget summaries."""
...
BUDGET = "25m"
REQUESTED_TRAIN_TOKENS = 25_000_000
...
def export_main_large_25m_summary(
    *,
    metrics_root: Path,
    output_json: Path,
    output_csv: Path,
    ...
    if missing:
        raise FileNotFoundError("Missing 25M summaries:\n" + "\n".join(missing))
    ...
    payload = {
        "scope": "main_large_25m_fixed_budget_results",
        "description": (
            "Curated results for the 25M-token subset of the main large fixed-budget "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("results/analysis/main_large_25m_summary.json"),
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("results/analysis/main_large_25m_summary.csv"),
    )
...
def main() -> None:
    args = parse_args()
    payload = export_main_large_25m_summary(
