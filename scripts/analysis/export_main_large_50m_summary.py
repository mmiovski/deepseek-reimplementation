"""Export curated main-large 50M fixed-budget summaries."""
...
def export_main_large_50m_summary(
    *,
    metrics_root: Path,
    output_json: Path,
    output_csv: Path,
    ...
    if missing:
        raise FileNotFoundError("Missing 50M summaries:\n" + "\n".join(missing))
    ...
    payload = {
        "scope": "main_large_50m_fixed_budget_results",
        "description": (
            "Curated results for the 50M-token subset of the main large fixed-budget "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("results/analysis/main_large_50m_summary.json"),
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("results/analysis/main_large_50m_summary.csv"),
    )
...
def main() -> None:
    args = parse_args()
    payload = export_main_large_50m_summary(
