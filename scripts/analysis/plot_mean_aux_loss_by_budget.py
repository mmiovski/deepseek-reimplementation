...
OUT_PNG = OUT_DIR / "mean_aux_loss_by_budget.png"

METRIC_NAME = "mean_aux_loss"
...
def load_from_descriptives() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    raise ValueError("No usable mean_aux_loss rows found in descriptives artifact.")
...
def load_from_summary_flat() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    raise ValueError("No usable mean_aux_loss values found in summary-flat artifact.")
...
def configure_axes(ax: Axes, plotted_values: list[tuple[float, float]]) -> None:
    ...
    ax.set_ylabel("Mean Auxiliary Loss")
