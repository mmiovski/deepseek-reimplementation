...
OUT_PNG = OUT_DIR / "mean_expert_load_variance_by_budget.png"

METRIC_NAME = "mean_expert_load_variance"
...
def load_from_descriptives() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    for row in rows:
        ...
        if model not in PLOT_MODEL_ORDER or budget not in BUDGET_ORDER:
            continue
    ...
    raise ValueError("No usable mean_expert_load_variance rows found in descriptives artifact.")
...
def load_from_summary_flat() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    raise ValueError("No usable mean_expert_load_variance values found in summary-flat artifact.")
...
def configure_axes(ax: Axes, plotted_values: list[tuple[float, float]]) -> None:
    ...
    ax.set_ylabel("Mean Expert Load Variance")
