...
OUT_PNG = OUT_DIR / "tokens_per_activated_parameter_by_budget.png"

METRIC_NAME = "tokens_per_activated_parameter"
...
def load_from_descriptives() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    raise ValueError(
        "No usable tokens_per_activated_parameter rows found in descriptives artifact."
    )
...
def load_from_summary_flat() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    raise ValueError(
        "No usable tokens_per_activated_parameter values found in summary-flat artifact."
    )
...
def configure_axes(ax: Axes, plotted_values: list[tuple[float, float]]) -> None:
    ...
    ax.set_ylabel("Tokens/Activated Parameter")
