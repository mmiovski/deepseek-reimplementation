...
OUT_PNG = OUT_DIR / "train_tokens_per_second_by_budget.png"

METRIC_NAME = "train_tokens_per_second"
...
def load_from_descriptives() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    raise ValueError("No usable train_tokens_per_second rows found in descriptives artifact.")
...
def load_from_summary_flat() -> dict[tuple[str, str], tuple[float, float]]:
    ...
    raise ValueError("No usable train_tokens_per_second values found in summary-flat artifact.")
...
def configure_axes(ax: Axes, plotted_values: list[tuple[float, float]]) -> None:
    ...
    ax.set_ylabel("Training Throughput (Tokens/s)")
